    ...
    init: function () {
      ...
      if (this.webview) {
        ...
        this.webview.addEventListener('new-window', (e) => {
          ...
          // 在当前页面打开
          // this.src = e.url
          this.$electron.remote.getCurrentWindow().loadURL(e.url)
        })
      }
    },
    checkParamsRedirect: function () {
      // 参数不存在则跳过去初始化
      let that = this
      that.src = that.$route.params.url ? that.$route.params.url : (this.$config.APP_URL || '')
      if (!that.src) {
